...
def _validate_times(start_time: datetime, end_time: datetime):
    ...
    if (end_time - start_time) > timedelta(hours=24):
        raise HTTPException(
            status_code=400,
            detail="Booking window too long",
        )
...
async def _resolve_user_vehicle(
    ...
    """
    User must have a registered vehicle before creating a booking.
    If vehicle_id is sent, validate it belongs to the logged-in user.
    If vehicle_id is not sent, use default/latest vehicle.
    """
    ...
    if not vehicle:
        raise HTTPException(
            status_code=400,
            detail="Please register a vehicle before booking a charger",
        )
...
async def create_booking(
    ...
    async with pool.acquire() as conn:
        ...
        if connector["station_id"] != request.station_id:
            raise HTTPException(
                status_code=400,
                detail="Connector does not belong to station",
            )

        if connector["status"] in ("offline", "maintenance"):
            raise HTTPException(
                status_code=400,
                detail=f"Connector is {connector['status']}",
            )
        ...
        if conflict_session:
            raise HTTPException(
                status_code=400,
                detail="Connector has an active session",
            )
